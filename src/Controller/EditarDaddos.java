    ...
    public void editaCamera(Vector list, int  cod) {
       ...
       for(int i = 0; i < list.size(); i++) {
           ...
           if(aux.getCodCorpo() == cod) {
               int numero = Validacoes.ValidarInt("Informe o novo numero da camara : ", 100, 1);
...
public void editaM(Vector list, int  cod) {
   ...
   for(int i = 0; i < list.size(); i++) {
       ...
       if(aux.getCodMorgue() == cod) {
           int conta = Validacoes.ValidarInt("Informe o novo numero de celular : ", 999999999, 20000);
           aux.setConta1(conta);
       } 
   }
}
...
public void editaC(Vector list, int  cod) {
   ...
   for(int i = 0; i < list.size(); i++) {
       ...
       if (ob instanceof CorpoConh) {
           aux = (CorpoConh)ob;
           if(aux.getCorpo() == cod) {
             String  nomePante = Validacoes.ValidarString("Informe o novo nome do parente : ", 999999999, 3);
...
public void editaD(Vector list, int  cod) {
   ...
   for(int i = 0; i < list.size(); i++) {
       ...
       if (ob instanceof CorpoDes) {
           aux = (CorpoDes)ob;
           if(aux.getCorpo() == cod) {
             String  marcasPele = Validacoes.ValidarString("Informe as novas marcas na pele  : ", 999999999, 3);
...
public void editaF(Vector list, int  cod) {
   ...
   for(int i = 0; i < list.size(); i++) {
       ...
       if(f.getCodF() == cod) {
           String funcao = Validacoes.ValidarString("Informe a nova funcao  : ", 1000, 3);
           f.setFuncao(funcao);
       }
   }
}
...
public void editaT(Vector list, int  cod) {
   ...
   for(int i = 0; i < list.size(); i++) {
       ...
       if(ct.getCodCorpo()  == cod) {
            int codMorgue  = Validacoes.ValidarInt("Informe o novo codigo da morgue : ", 9999, 1000); 
